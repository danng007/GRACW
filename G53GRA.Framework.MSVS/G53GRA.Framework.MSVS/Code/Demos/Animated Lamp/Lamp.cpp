...
{
	animateTime += deltaTime;                   // update the time and rotation steps
	animateRotation += deltaTime;

	if (animateTime >= 3.0)                    // check if we hit the end of the animation (3 seconds)
	{                                           // if so reset
		animateTime = 0.0;
		keyframe = -1.0;
	}

	if (animateTime < 1.0)                     // check if we are in the 1st second of animation
	{
	...
	else if (animateTime < 1.25)               // check if we are in the 1.0 to 1.25 seconds of animation
	{
		...
		*/
		...
		animateTranslation = 4.0*9.81*(animateTime - 1.0) + 0.5*(animateTime - 1.0)*(animateTime - 1.0)*4.0*GRAVITY;
	}
