...
void Water::Display()
{
	...
	glPushAttrib(GL_ALL_ATTRIB_BITS);              // Set the material properties of the waters surface
		...
		glPushMatrix();
			...
			y = 0.f; z = -0.5f; // initialise the start position as the top left coorner of the unit water mesh
			...
			for (int j = 0; j < zGridDims; j++)
			{
				...
				{
					glNormal3f(0.f, 1.f, 0.f);  // specify the quads normal
					...
					glVertex3f(x, y, z + 1.f / (float)zGridDims);
					...
					glVertex3f(x + 1.f / (float)xGridDims, y, z + 1.f / (float)zGridDims);
					...
					glVertex3f(x + 1.f / (float)xGridDims, y, z);

					x += 1.f / (float)xGridDims; // move the position of the quad on by one xgriddim step in the x axis
				}
				z += 1.f / (float)zGridDims; // move the position of the quad on by one zgriddim step in the z axis
			}
			glEnd();
...
void Water::Update(const double& deltaTime)
{
	...
	radius = sqrtf((1.f / xGridDims)*(1.f / xGridDims) + (1.f / zGridDims)*(1.f / zGridDims));
	radius /= 4.f; // make sure this radius does not intersect any of the other texture locations
